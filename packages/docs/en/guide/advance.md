...
When multiple syntaxes are displayed in a `<demo />` component, the default display order is `vue,react,svelte,solid,preact,html` and the first syntax in the display order is selected by default.
...
If your demo contains multiple files, you can use the `vueFiles/reactFiles/svelteFiles/solidFiles/preactFiles/htmlFiles` property to specify the files and codes of the corresponding type of demo to be displayed. For example:
...
You can also specify file names by passing an `Object` to the `vueFiles/reactFiles/svelteFiles/solidFiles/preactFiles/htmlFiles` property. For example, if you only want to display each file's `basename` without its directory hierarchy, you can do it as follows:
...
You can also use the `vueFiles/reactFiles/svelteFiles/solidFiles/preactFiles/htmlFiles` property to completely customize the files and code to be displayed. For example, if you want to display two formats of a code, `typescript` and `javascript`, you can do it as follows:
...
`vitepress-demo-plugin` internally treats `vueFiles/reactFiles/svelteFiles/solidFiles/preactFiles/htmlFiles` as a string of type `string[] | Record<string, string>`, so you can only declare its value directly, and cannot replace it by referencing variables.
